...
- **DEPLOYED + LIVE on NAS 2026-05-26:** image rebuilt `--no-cache` (timezone fix + all new collectors); `scripts/nas/run_job.sh` dispatcher added (full `/usr/local/bin/docker` path + network create/connect + the `intel:intel@trading-intel-pg:5432` DATABASE_URL); `vix_snapshot` smoke-tested EXIT 0 (now writes vix9d/vix3m/vix6m/vrp). **NAS timezone fixed to America/New_York** (was UTC-5/EST, 1h off in DST — DSM schedules fire on NAS clock so this mattered for intraday windows). Migrations 0013/0014/0015 applied. `.N/.TO` cleanup ran (7 of 114 rows normalized). **All DSM tasks now present** incl. greeks_snapshot/chain_snapshot/gex_rolling + the 4 new (delta_flow 5-min RTH, live_gex 10-min RTH, prune_live_gex 02:30, vol_richness 16:40). Everything this session is now collecting.
