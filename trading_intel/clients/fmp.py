...
The only module that talks to FMP (CLAUDE.md rule 1). Free tier is ~250 calls/day,
so callers are sparing. Every endpoint is best-effort and degrades to None / []
on failure. Descriptive research input only (rule 4). Key from ``settings.FMP_API``.
"""
...
class FmpClient:
    """Fetch FMP company data. Inject an httpx-like ``client`` in tests."""
    ...
    def profile(self, ticker: str) -> dict | None:
        """Company profile (name, sector, industry, description, mktCap, ...)."""
    ...
    def income_statement(self, ticker: str, *, limit: int = 2) -> list[dict]:
        ...
        data = self._get(f"income-statement/{ticker}", period="annual", limit=limit)
