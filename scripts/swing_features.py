#!/usr/bin/env python3
"""Daily swing feature-snapshot collector -> ``swing_features``.

Banks the per-name Stage-1 feature vector each trading day so the percentile
features (IV-rank, IV/RV, skew, GEX/DEX positioning) mature into a real trailing
distribution for the Stage-2 fitted model (see the swing-trade-system build).
Fed by CVForge (ADR-004); convexlib is untouched. Percentiles are standardized
against the name's own trailing 252d from this table (today excluded), matching
the ``skew_snapshots`` contract exactly.
...
NOTE: the feature extraction mirrors ``scripts/swing_report.py::analyze`` (minus
scoring). Consolidate the two into ``trading_intel/swing`` when P3 lands.
...
# Non-key columns refreshed by the upsert's DO UPDATE.
_UPDATE_COLS = (
    "spot",
    "atm_iv",
    "rv20",
...
def main() -> None:
    ...
    try:
        with session_factory() as session:
            records = build_records(session, client, symbols, as_of=as_of)
            _upsert(session, records)
            session.commit()
    finally:
        client.close()
    print(f"swing_features: upserted {len(records)} rows for {as_of.isoformat()}")
